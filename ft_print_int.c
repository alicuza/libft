...
/*   Updated: 2025/12/14 02:27:54 by sancuta          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	print_int(t_form_spec *data, int n)
{
	...
	ft_memset(written, 0, 3);
	ft_prep_int(data, n, *nb_len, *sign_len);
	if (!(data->flag & (FLAG_MINUS | FLAG_ZERO)))
		written[0] = put_space(data, *nb_len, *sign_len);
	if (data->flag & FLAG_SIGN)
		written[1] = put_sign(data);
	if (data->flag & (FLAG_ZERO | FLAG_DOT))
		written[2] = put_zero(data, *nb_len, *sign_len);
	if (nb_len)
	{
		if (data->flag & FLAG_NEG)
			written[3] = put_nbr_base((unsigned long)(-(long)n), "0123456789");
		else
			written[3] = put_nbr_base((unsigned long)n, "0123456789");
	}
